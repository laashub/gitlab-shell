...
func TestCustomUploadPack(t *testing.T) {
	repo := "group/repo"
	keyId := "1"
	...
	requests := []testserver.TestRequestHandler{
				...
				body := map[string]interface{}{
					...
					"payload": map[string]interface{}{
						...
						"data": map[string]interface{}{
							"api_endpoints": []string{"/geo/proxy_git_ssh/info_refs_upload_pack", "/geo/proxy_git_ssh/upload_pack"},
							"gl_username":   "custom",
		...
		{
			Path: "/geo/proxy_git_ssh/info_refs_upload_pack",
			Handler: func(w http.ResponseWriter, r *http.Request) {
				b, err := ioutil.ReadAll(r.Body)
				require.NoError(t, err)
		...
		{
			Path: "/geo/proxy_git_ssh/upload_pack",
			Handler: func(w http.ResponseWriter, r *http.Request) {
				b, err := ioutil.ReadAll(r.Body)
				require.NoError(t, err)
	...
	cmd := &Command{
		...
		Args:       &commandargs.Shell{GitlabKeyId: keyId, CommandType: commandargs.UploadPack, SshArgs: []string{"git-upload-pack", repo}},
